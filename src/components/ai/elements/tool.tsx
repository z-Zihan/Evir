...
/* eslint-disable react-refresh/only-export-components -- getStatusBadge helper is exported alongside components, as upstream */
/**
 * Vendored from vercel/ai-elements `packages/elements/src/tool.tsx`
 * (Apache-2.0). Adaptations from the upstream source:
 * - `@repo/shadcn-ui/*` imports mapped to Evir's `components/ui`;
 * - `ToolUIPart`/`DynamicToolUIPart` types come from the local ai-types shim
 *   instead of the `ai` runtime package;
...
export const ToolOutput = ({ className, output, errorText, ...props }: ToolOutputProps) => {
  ...
  if (typeof output === "object" && !isValidElement(output)) {
    Output = <Code code={JSON.stringify(output, null, 2)} />;
    ...
    <div className={cn("space-y-2", className)} {...props}>
      ...
      <div
        className={cn(
          ...
          errorText ? "bg-destructive/10 text-destructive" : "bg-surface-hover/50 text-foreground",
        )}
      >
...
/** Local stand-in for the upstream CodeBlock dependency (documented). */
function Code({ code }: { code: string }) {
  return <pre className="overflow-x-auto p-3 font-mono text-[11.5px] leading-relaxed">{code}</pre>;
}
