...
| --------------------- | --------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| **Agent Verified**    | Latest full-suite (20-task) real-endpoint Golden Agent Tasks run passed     | 智谱 BigModel / GLM                                                                       |
| **Smoke Verified**    | Latest real-endpoint eval passed at 10-task smoke scale, full suite pending | None yet                                                                                  |
...
The only model-level Agent Verified evidence today: `evomap-deepseek-v4-flash` (a DeepSeek-family model reached through the EvoMap third-party gateway via the openai-compatible-chat protocol on the Zhipu preset) ran the full 20-task required Golden Agent Tasks suite on 2026-09-09 — 16/20 passed (0.8), tool-call success 0.891, 0 unauthorized operations, 0 out-of-scope changes. The full history (an earlier 17/20 run the same day with 2 out-of-scope files was honestly recorded as partial and never hides behind the later pass) lives in `src/core/providers/provider-validation.json`. **This evidence belongs to that model alone** — it does not represent GLM models (evidence never transfers across models); GLM models are Protocol Verified until their own full-suite real evals say otherwise. Historical manual real-machine QA logs also live in [Release Readiness](docs/release-readiness.md).
