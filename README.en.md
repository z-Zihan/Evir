...
| Tier                  | Meaning                                                                                           | Vendors / models                                                                          |
| --------------------- | ------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| **Agent Verified**    | Latest full-suite (20-task) real-endpoint eval passed at ≥90% with zero unauthorized/out-of-scope | None yet                                                                                  |
| **Eval Candidate**    | Latest real eval qualified (≥80%, zero unauthorized/out-of-scope) but below the ≥90% verified bar | 智谱 BigModel / GLM · evomap-deepseek-v4-flash (EvoMap gateway, DeepSeek-family)          |
| **Smoke Verified**    | Latest real-endpoint eval passed at 10-task smoke scale (≥90%), full suite pending                | None yet                                                                                  |
...
The only model-level real-eval evidence today: `evomap-deepseek-v4-flash` (a DeepSeek-family model reached through the EvoMap third-party gateway via the openai-compatible-chat protocol on the Zhipu preset) ran the full 20-task required Golden Agent Tasks suite on 2026-09-09 — 16/20 passed (0.8), tool-call success 0.891, 0 unauthorized operations, 0 out-of-scope changes. Under the ≥90% Agent Verified bar (re-reviewed 2026-09-11) that latest run holds **Eval Candidate** — no model has reached Agent Verified yet. The full history (an earlier 17/20 run the same day with 2 out-of-scope files was honestly recorded as partial and never hides behind the later pass) lives in `src/core/providers/provider-validation.json`. **This evidence belongs to that model + that endpoint alone** — it represents neither GLM models nor Zhipu's official endpoint (evidence never transfers across models or endpoints); GLM models are Protocol Verified until their own full-suite real evals say otherwise. Historical manual real-machine QA logs also live in [Release Readiness](docs/release-readiness.md).
...
- **Harness safety / runtime eval (deterministic)**: 20 Golden Agent Tasks on a frozen fixture repo (`pnpm test:agent-eval`) — scripted model over the real execution stack, covering permission boundaries, unauthorized operations (must be 0), out-of-scope changes (must be 0), stop, recovery, and completion evidence; currently 20/20, wired into CI.
- **Real model eval (model-level, endpoint-aware, latest-run decides)**: the same tasks run against real endpoints; every result (pass/fail/partial) enters the `provider-validation.json` history and the latest run sets the tier. A 10-task smoke run can earn at most Smoke Verified — never full verification. Current results are in the tier table above (EvoMap gateway `evomap-deepseek-v4-flash`, full suite 16/20 → Eval Candidate); details in [eval/README](eval/README.md).
...
Evir is under active development and **not released yet** (no LICENSE file — see below). The core path (chat, agent tools and approvals, Plan/Goal, permission profiles, snapshot rollback, MCP connections, logging and diagnostics export) is implemented. **Per-item verification status (including NOT RUN / BLOCKED lists) is owned by [Release Readiness](docs/release-readiness.md)**: real Windows machines and external MCP servers are not yet verified; upgrade/downgrade has 5+ recorded DMG-overwrite PASS runs; 30–60 minute long tasks have real samples (interrupt+resume PASSes and honestly-rejected FAILs) but **no single continuous ≥30-minute PASS yet**.

> **Install notes (honest)**: current macOS installers are ad-hoc signed and not notarized. macOS Gatekeeper may block opening them directly — allow manually in System Settings → Privacy & Security (or right-click → Open). This project does not ship a regular Windows installer yet.
