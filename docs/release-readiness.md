...
| 项                                                                                                 | 状态                                                                           | 证明                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| -------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Real EvoMap 网关（zhipu preset · openai-compatible · `evomap-deepseek-v4-flash`，DeepSeek 系模型） | PASS                                                                           | 真实 Ask/流式/停止/恢复/错误分类/审批/写盘（2026-08-27/28/29 多轮，当时误记为"GLM"）；2026-09-08 真实 Golden Agent Tasks 10 任务 9/10（模型级证据，Smoke Verified 档；见 eval/results/real-latest.json 与 provider-validation.json）                                                                                                                                                                                                                                                                                                                                |
...
| 30–60min 长任务中断+续跑（2026-09-09 本轮）                                                        | PASS(无头 run1/2 + GUI 交付物复核) / FAIL(run3/4 + GUI run 级终态，均如实拒绝) | 无头四样本（eval/long-task，真实 EvoMap GLM）：run1 PASS 24.7min（真中断→断点自证→无重复副作用→复跑绿）；run2 PASS 18.2min（幂等续跑）；run3/4 FAIL（停滞/流错误被拒）——断言 0 假 PASS。GUI 安装版：toolsForNode 死分支暴露→修复重建→锁屏 11h→**4 次续跑**（审批交互 AX 实操）→验证节点 run_command 实证放行（TYPECHECK_OK/TESTS_OK）→交付物独立复核全绿（14/14 测试、tsc PASS、范围仅 packages/cli、schema 小节恰 1 次、package.json 零改动）；run 级终态诚实 FAIL（GUI 子进程 PATH 无 pnpm——改进项：工具沙箱 PATH 注入）。单次连续 ≥30min PASS 样本未取得（如实） |
...
| 档                 | 当前厂商与模型                                                                        | 依据                                                           |
| ------------------ | ------------------------------------------------------------------------------------- | -------------------------------------------------------------- |
| Agent Verified     | 暂无                                                                                  | 待 20 任务 required suite 最新一次达标                         |
| Smoke Verified     | 智谱 preset · `evomap-deepseek-v4-flash`（EvoMap 网关，DeepSeek 系，2026-09-08 9/10） | 10 任务冒烟规模达标；**不代表 GLM 系模型**（模型级证据不互借） |
