...
| 30–60min 长任务中断+续跑（2026-09-09 本轮）    | PASS(无头) / PENDING(GUI) | 无头真实档：产品同一 runAgentLoop，15min 处 AbortSignal 真实中断后同会话续跑，README schema 恰好一次、变更仅 packages/cli、CLI 测试+typecheck 全绿（eval/long-task）。GUI 安装版同任务在验证节点暴露 toolsForNode 死分支缺陷（run_command 被 capability 过滤提前排除），已修复+回归+重建覆盖安装；GUI 续跑因会话屏幕锁定未执行（NOT RUN，如实记录） |
