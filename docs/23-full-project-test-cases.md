...
>
> 文档性质：**测试用例目录（Canonical）**。测试层级与触发策略见 `docs/testing.md`；当前逐项验证状态见 `docs/release-readiness.md`；历史执行记录（2026-08-25/26）已移至 `docs/archive/test-execution-records-2026-08.md`。
> 测试文件存在或构建成功不等于产品闭环；真实 Provider、原生宿主与跨平台结果必须分别取证。签名/公证为可选增强（P2）：ad-hoc 非签名包是默认交付物，实体机安装与升级取证仍必须完成。
...
| ENV-003 | P1  | A+B  | 分别以 Web/Desktop 启动       | Web 仅 Ask；Desktop 侧栏 PROJECTS/CHATS，Project Thread 默认任务按需使用 Agent 工具且显式提供 Plan/Goal，Standalone Chat 恒为 Ask |
...
| PRO-019 | P0  | A+N  | Web/Desktop/VS Code Secret 边界 | Web 默认内存；Desktop 加密 Vault（本地加密文件）；VS Code SecretStorage；均不进配置 JSON |
...
# Rust（测试已封装为 pnpm test:rust；fmt/clippy 仍直接调用 cargo）
pnpm test:rust
