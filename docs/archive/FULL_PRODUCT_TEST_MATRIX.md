...
| Web/Desktop frontend | Settings            | 全入口与偏好持久化             | 12 settings pages                    | 逐页打开、切换语言主题   | 页面可达，刷新后偏好保留               | E2E           | Yes       | PASS    | `comprehensive-qa.spec.ts`            | —      |
...
| Desktop frontend     | Goal Workbench      | 生命周期与完成条件             | execution/paused/blocked/partial     | pause/resume/stop/retry  | 控件与 doneWhen 证据状态正确           | Component     | Yes       | PASS    | `TaskWorkbench.test.tsx` 19/19        | —      |
| Desktop frontend     | Project Permission  | 三档权限与高风险边界           | ask/workspace/full + L4              | 切换权限并执行策略判定   | Full 首次确认；L4 始终逐次审批         | UI/unit       | Yes       | PASS    | permission tests 13/13                | QA-009 |
...
| Web/Desktop frontend | Conversation        | 500 条消息与边界语料           | long URL/Unicode/RTL/table/code      | 渲染与滚动               | 消息完整且窄屏无横向溢出               | E2E           | Yes       | PASS    | comprehensive 2 platforms             | —      |
| Web/Desktop frontend | Provider            | HTTP/流式异常矩阵              | 401/403/404/500/disconnect/malformed | 发送后继续交互           | 错误可见且恢复，不崩溃                 | E2E           | Yes       | PASS    | comprehensive 12 checks               | —      |
| Web/Desktop frontend | Attachments         | 多选、删除、上限、非法类型     | empty/multiple/binary                | 选择与移除               | 不丢附件，限制与错误可见               | E2E/unit      | Yes       | PASS    | comprehensive + chat-store            | —      |
...
| Core                 | Quality gate        | format/lint/type/tests         | current diff                         | `pnpm check`             | 全部通过                               | Static/unit   | Yes       | PASS    | 108 files / 675 + VS Code 8 + CLI 8   | QA-006 |
| Core/Desktop         | Project filesystem  | 路径边界与移动重绑             | missing/symlink/../Unicode/deep      | 校验、重绑、模拟重启     | 越界拒绝，Project/Thread 关联保留      | Unit/Rust     | Yes       | PASS    | workspace + project-store tests       | —      |
| Core/Desktop         | Verification        | 退出码/超时/取消证据           | checker failed/timed out/cancelled   | 自动验证                 | 保留真实状态与退出码                   | Unit          | Yes       | PASS    | `verification.test.ts` 6/6            | QA-008 |
| Core                 | Context/stream/race | ownership/error/finally        | edge cases                           | 单元与 E2E               | 不串流、不锁死、可恢复                 | Unit/E2E      | Yes       | PASS    | 108 test files + stress               | —      |
| Web                  | Production build    | release assets                 | production                           | `pnpm build:web`         | 构建成功                               | Build         | Yes       | PASS    | Vite 2229 modules                     | —      |
| Desktop              | Production build    | arm64 app/DMG                  | release                              | `pnpm build:desktop`     | 生成 app 与 DMG                        | Build/package | Yes       | PASS    | 6.46 MiB current DMG                  | —      |
...
| Tauri/Rust           | Backend             | fmt/clippy/test                | all targets/features                 | 执行门禁                 | 无 warning，测试通过                   | Static/unit   | Yes       | PASS    | Rust 37/37                            | QA-007 |
| Tauri/Rust           | Shell output        | 大输出命令                     | stdout > 200 KB                      | 执行并读取               | 完整排空管道且有界保留输出             | Unit          | Yes       | PASS    | `command_pipe_is_fully_drained...`    | QA-007 |
| Native macOS         | Release app         | 原生关键点击链路               | pre-final rebuild release app        | AX 读取、点击、Escape    | 可打开添加供应商/设置并关闭            | Native UI     | Yes       | PASS    | 本轮 AX；最终包另有 build 证据        | —      |
